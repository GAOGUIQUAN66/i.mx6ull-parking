...
class CardPredictor:
	def __init__(self):
		...
		f = open(os.path.join(BASE_DIR, 'config.js'), encoding='utf-8')
		j = json.load(f)
		for c in j["config"]:
			if c["open"]:
				self.cfg = c.copy()
	...
	def train_svm(self):
		...
		svm_chinese_path = os.path.join(BASE_DIR, "svmchinese.dat")
		...
		if os.path.exists(svm_chinese_path):
			self.modelchinese.load(svm_chinese_path)
		else:
			chars_train = []
			chars_label = []
	...
	def save_traindata(self):
		...
		if not os.path.exists(svm_chinese_path):
			self.modelchinese.save(svm_chinese_path)
